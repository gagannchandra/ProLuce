  ...
  return (
    <div className="w-full">
      ...
      <div className="md:hidden space-y-3">
        {products.map((p) => (
          <MobileCatalogCard key={p.id} product={p} onOpenQuote={onOpenQuote} />
        ))}
      </div>

      {/* Desktop Architectural Data Table (>= md) */}
      <Card className="hidden md:block w-full overflow-hidden border-border bg-card shadow-xs">
