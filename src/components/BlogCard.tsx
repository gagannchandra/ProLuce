...
export default function BlogCard({ post }: { post: BlogPost }) {
  ...
  return (
    <Card className="group flex flex-col overflow-hidden rounded-2xl border border-border/80 bg-card/90 backdrop-blur-xs transition-all duration-300 hover:border-border hover:shadow-xl dark:hover:border-[#e6dfd1]/40">
      ...
      <CardHeader className="p-6 pb-2">
        ...
        <Link href={`/blog/${post.slug}`} className="block focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-stone-400 rounded-sm">
          <h3 className="font-display text-2xl lg:text-[25px] font-light leading-snug mt-2.5 text-foreground group-hover:text-stone-600 dark:group-hover:text-[#f4f0e6] transition-colors">
            {post.title}
          </h3>
        </Link>
      </CardHeader>
      ...
      <CardContent className="p-6 pt-0 flex-1 flex flex-col justify-between">
        ...
        <Button asChild variant="outline" size="sm" className="w-fit font-sans text-xs uppercase tracking-[0.14em] font-medium rounded-full px-4 h-8 gap-1.5 group/btn border-border/80 hover:border-border hover:bg-accent transition-all">
